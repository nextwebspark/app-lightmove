...
`cd apps/web && npm run build && npx vitest` — including the mapping step rendering, a correction
reaching the commit call, and a custom column rendering as a grid column with its values on the rows.
