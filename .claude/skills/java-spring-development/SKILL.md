...
workspace/                 # feature template — project / position / strategy / triagecompany / candidate copy this
...
- `workspace`'s `OnboardingService` calls `ApolloCompanyQueryService.byAccountIds` the same way, to
  ...
  typeahead through `CompanySuggestionSearch` — the one validator both typeahead endpoints share —
  behind its own `RateLimitGuard` budget, because `/companies/search` needs `PROJECT_BROWSE`, which
  nobody holds before their workspace exists.
