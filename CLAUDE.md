...
in `apps/web/public/brand` (`favicon.svg`, both `BrandMark`s, the extension's icons and the email's
`uncava-mark-email-v1.png` are drawn from that one geometry) and every user-facing string says Uncava,
