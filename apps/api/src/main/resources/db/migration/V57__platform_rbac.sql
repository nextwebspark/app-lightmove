...
--
-- Written as V51 and renumbered before it merged: main reached V56 first, and Flyway refuses a
-- version below the newest one applied. The guards below are for a dev database that ran it as V51.

ALTER TABLE app_lm_role DROP CONSTRAINT IF EXISTS app_lm_role_scope_chk;
ALTER TABLE app_lm_role
    ADD CONSTRAINT app_lm_role_scope_chk CHECK (scope IN ('WORKSPACE', 'PROJECT', 'PLATFORM'));

ALTER TABLE app_lm_action DROP CONSTRAINT IF EXISTS app_lm_action_scope_chk;
...
CREATE TABLE IF NOT EXISTS app_lm_user_platform_role (
