...
-- super admin (V57); a non-null row is one firm's, edited by its workspace admin. A firm's row sharing a
...
--
-- Written as V52 and renumbered before it merged: main reached V56 first, and Flyway refuses a
-- version below the newest one applied. The guards below are for a dev database that ran it as V52.

ALTER TABLE app_lm_position_template
    ADD COLUMN IF NOT EXISTS revised_at      timestamptz NOT NULL DEFAULT now(),
    ADD COLUMN IF NOT EXISTS revised_by      uuid REFERENCES app_lm_user (id) ON DELETE SET NULL,
    ADD COLUMN IF NOT EXISTS customised_from timestamptz;
...
CREATE TABLE IF NOT EXISTS app_lm_position_template_hidden (
