...
public record OAuthQuirkSettings(
         ...
         *
         * <p>The nonce binds the id_token to this browser's authorisation request. Dropping it
         * leaves the code exchange as that binding — single-use, server-to-server over TLS with our
         * secret — and {@code state} still covers CSRF. Only for a provider that gives no choice.
         */
         ...
         *
         * <p>The claim is optional in OIDC and Spring coerces anything present to a Boolean, so
         * "absent" is the only ambiguous case. Trusting absence by default would be a standing
         * account-takeover primitive the day someone adds a self-hosted IdP: this service links a
         * provider identity to an existing account on a matching email, so an IdP that lets anyone
         * claim {@code victim@firm.com} would hand over that person's workspace. Providers that prove
         * the address by other means opt in here, one reviewable line at a time.
         */
