...
export default defineConfig(({ mode }) => {
  // Loaded with an empty prefix, as apps/extension does, so APP_VERSION is visible without a VITE_
  // prefix — it is a build parameter, not something the bundle reads for itself. Only the key named
  // below is ever inlined; an empty prefix returns the whole environment.
